...
def render_status(root: Path) -> str:
    ...
    try:
        ...
        if scope["mode"] == "sprint":
            out += [f"## {scope['milestone']} · {scope['id']}", "", scope["goal"], "",
                    f"{scope['done']}/{scope['current']} passed G5; {scope['committed']} committed at start; "
                    f"+{scope['added']}/−{scope['removed']} scope change. "
                    f"{scope['in_test']} await G4; {scope['beta']} await G5.", ""]
            out += [f"Demo: `{scope['demo']}`", ""]
            if scope.get("preview"):
                out += [f"Early frontend preview: {scope['preview']} (backend-independent).", ""]
        else:
            out += [scope["message"], ""]
    except (WorkflowError, KeyError, TypeError) as exc:
        out += [f"Delivery plan error: {exc}", ""]
    schema_errors = [f"{t['id']}: {p}" for t in tasks for p in t.get("problems", [])]
    if schema_errors:
        out += ["Task integrity errors — counts are provisional:", ""]
        out += [f"- {p}" for p in schema_errors] + [""]
